 ...
 * overwrite any installed event handlers on the given Occamy.InputStream.
 * 
 * @constructor
 * @param {Occamy.InputStream} stream The stream that data will be read
 *                                       from.
 */
Occamy.ArrayBufferReader = function(stream) {

    /**
     * Reference to this Occamy.InputStream.
     * @private
     */
 ...
 * 
 * @constructor
 * @param {Occamy.OutputStream} stream The stream that data will be written
 *                                        to.
 */
Occamy.ArrayBufferWriter = function(stream) {

    /**
     * Reference to this Occamy.StringWriter.
     * @private
     */
    ...
    /**
     * The maximum length of any blob sent by this Occamy.ArrayBufferWriter,
     * in bytes. Data sent via
     * [sendData()]{@link Occamy.ArrayBufferWriter#sendData} which exceeds
     * this length will be split into multiple blobs. As the Occamy protocol
     ...
     *
     * @type {Number}
     * @default {@link Occamy.ArrayBufferWriter.DEFAULT_BLOB_LENGTH}
     */
    this.blobLength = Occamy.ArrayBufferWriter.DEFAULT_BLOB_LENGTH;
     ...
     * @event
     * @param {Occamy.Status} status The status of the operation.
     */
    this.onack = null;
...
/**
 * The default maximum blob length for new Occamy.ArrayBufferWriter
 * instances.
 *
 * @constant
 * @type {Number}
 */
Occamy.ArrayBufferWriter.DEFAULT_BLOB_LENGTH = 6048;

/**
 * A reader which automatically handles the given input stream, assembling all
 * received blobs into a single blob by appending them to each other in order.
 * Note that this object will overwrite any installed event handlers on the
 * given Occamy.InputStream.
 * 
 * @constructor
 * @param {Occamy.InputStream} stream The stream that data will be read
 *                                       from.
 ...
 */
Occamy.BlobReader = function(stream, mimetype) {
    ...
    /**
     * The length of this Occamy.InputStream in bytes.
     * @private
     */
    var length = 0;

    // Get blob builder
    var blob_builder = new (function() {
    ...
    /**
     * Returns the current length of this Occamy.InputStream, in bytes.
     * @return {Number} The current length of this Occamy.InputStream.
     */
    ...
    /**
     * Returns the contents of this Occamy.BlobReader as a Blob.
     * @return {Blob} The contents of this Occamy.BlobReader.
     */
    this.getBlob = function() {
 ...
 *
 * @constructor
 * @param {Occamy.OutputStream} stream
 *     The stream that data will be written to.
 */
Occamy.BlobWriter = function BlobWriter(stream) {

    /**
     * Reference to this Occamy.BlobWriter.
     *
     * @private
     * @type {Occamy.BlobWriter}
     */
    var guacWriter = this;

    /**
     * Wrapped Occamy.ArrayBufferWriter which will be used to send any
     * provided file data.
     *
     * @private
     * @type {Occamy.ArrayBufferWriter}
     */
    var arrayBufferWriter = new Occamy.ArrayBufferWriter(stream);
    ...
    this.sendBlob = function sendBlob(blob) {
         ...
         * [sendBlob()]{@link Occamy.BlobWriter#sendBlob}. The chunk itself
     ...
     * [sendBlob()]{@link Occamy.BlobWriter#sendBlob}. The transfer for the
     ...
     * via [sendBlob()]{@link Occamy.BlobWriter#sendBlob}.
     *
     * @event
     * @param {Blob} blob
     ...
     * [sendBlob()]{@link Occamy.BlobWriter#sendBlob} has finished being
     * sent.
     *
     * @event
     * @param {Blob} blob
...
/**
 * Occamy protocol client. Given a {@link Occamy.Tunnel},
 * automatically handles incoming and outgoing Occamy instructions via the
 * provided tunnel, updating its display using one or more canvas elements.
 * 
 * @constructor
 * @param {Occamy.Tunnel} tunnel The tunnel to use to send and receive
 *                                  Occamy instructions.
 */
Occamy.Client = function(tunnel) {
    ...
    /**
     * The underlying Occamy display.
     *
     * @private
     * @type {Occamy.Display}
     */
    var display = new Occamy.Display();

    /**
     * All available layers and buffers
     *
     * @private
     * @type {Object.<Number, (Occamy.Display.VisibleLayer|Occamy.Layer)>}
     */
    var layers = {};
    ...
    var stream_indices = new Occamy.IntegerPool();
    ...
    /**
     * Produces an opaque representation of Occamy.Client state which can be
    ...
    /**
     * Restores Occamy.Client protocol and display state based on an opaque
     * object from a prior call to exportState(). The Occamy.Client instance
     * used to export that state need not be the same as this instance.
     *
     * @param {Object} state
     *     An opaque representation of Occamy.Client state from a prior call
     *     to exportState().
     *
     * @param {function} [callback]
    ...
    this.importState = function importState(state, callback) {
        ...
        for (key in state.layers) {
            ...
            if (importLayer.url) {
                display.setChannelMask(layer, Occamy.Layer.SRC);
    ...
    /**
     * Returns the underlying display of this Occamy.Client. The display
     * contains an Element which can be added to the DOM, causing the
     * display to become visible.
     * 
     * @return {Occamy.Display} The underlying display of this
     *                             Occamy.Client.
     */
     ...
     * state.
     * 
     * @param {Occamy.Mouse.State} mouseState The state of the mouse to send
     *                                           in the mouse event.
     */
     ...
     * Occamy.OutputStream using that index, associating the resulting
     ...
     *     A new Occamy.OutputStream with a newly-allocated index and
     *     associated with this Occamy.Client.
     */
    this.createOutputStream = function createOutputStream() {
        ...
        // Return new stream
        var stream = output_streams[index] = new Occamy.OutputStream(guac_client, index);
        return stream;
     ...
     * Occamy connection will be notified via an "end" instruction that the
    ...
    /**
     * Fired whenever the state of this Occamy.Client changes.
     * 
     * @event
     ...
     * 
     * @event
     * @param {Occamy.Status} status A status object which describes the
     *                                  error.
     */
    this.onerror = null;
     ...
     * 
     * @event
     * @param {Occamy.InputStream} stream The stream that will receive data
     *                                       from the server.
     ...
     *
     * @return {Occamy.Display.VisibleLayer|Occamy.Layer}
     ...
     *
     * @param {Occamy.Display.VisibleLayer|Occamy.Layer} layer
    ...
    function getParser(index) {
        ...
        if (parser == null) {
            parser = parsers[index] = new Occamy.Parser();
    ...
    /**
     * Handlers for all instruction opcodes receivable by a Occamy protocol
     * client.
     * @private
     */
    var instructionHandlers = {
        ...
        "ack": function(parameters) {
            ...
            if (stream) {
                ...
                if (stream.onack)
                    stream.onack(new Occamy.Status(code, reason));
        ...
        "body" : function handleBody(parameters) {
            ...
            if (object && object.onbody) {
                var stream = streams[streamIndex] = new Occamy.InputStream(guac_client, streamIndex);
        ...
        "error": function(parameters) {
            ...
            if (guac_client.onerror)
                guac_client.onerror(new Occamy.Status(code, reason));
        ...
        "img": function(parameters) {
            ...
            // Create stream
            var stream = streams[stream_index] = new Occamy.InputStream(guac_client, stream_index);
            var reader = new Occamy.DataURIReader(stream, mimetype);
        ...
        "transfer": function(parameters) {
            ...
            /* Anything else that isn't a NO-OP */
                ...
                display.transfer(srcL, srcX, srcY, srcWidth, srcHeight, 
                    dstL, dstX, dstY, Occamy.Client.DefaultTransferFunction[function_index]);
    ...
    /**
     * Connects the underlying tunnel of this Occamy.Client, passing the
     * given arbitrary data to the tunnel during the connection process.
     *
     * @param data Arbitrary connection data to be sent to the underlying
     *             tunnel during the connection process.
     * @throws {Occamy.Status} If an error occurs during connection.
     */
...
/**
 * Map of all Occamy binary raster operations to transfer functions.
 * @private
 */
Occamy.Client.DefaultTransferFunction = {
 ...
 * 
 * @constructor
 * @param {Occamy.InputStream} stream
 *     The stream that data will be read from.
 */
Occamy.DataURIReader = function(stream, mimetype) {

    /**
     * Reference to this Occamy.DataURIReader.
     * @private
     */
    var guac_reader = this;
...
/**
 * The Occamy display. The display does not deal with the Occamy
 ...
 * @constructor
 */
Occamy.Display = function() {

    /**
     * Reference to this Occamy.Display.
     * @private
     */
    ...
    var default_layer = new Occamy.Display.VisibleLayer(displayWidth, displayHeight);

    // Create cursor layer
    var cursor = new Occamy.Display.VisibleLayer(0, 0);
    cursor.setChannelMask(Occamy.Layer.SRC);
     ...
     * the location of the cursor image within the Occamy display, as
    ...
    /**
     * Fired when the default layer (and thus the entire Occamy display)
     * is resized.
     * 
     * @event
     * @param {Number} width The new width of the Occamy display.
     * @param {Number} height The new height of the Occamy display.
     */
    this.onresize = null;
    ...
    /**
     * Returns the element which contains the Occamy display.
     * 
     * @return {Element} The element containing the Occamy display.
     */
    ...
    /**
     * Returns the default layer of this display. Each Occamy display always
     ...
     * 
     * @return {Occamy.Display.VisibleLayer} The default layer.
     */
    ...
    /**
     * Returns the cursor layer of this display. Each Occamy display contains
     ...
     * 
     * @return {Occamy.Display.VisibleLayer} The cursor layer.
     */
     ...
     * 
     * @return {Occamy.Display.VisibleLayer} The newly-created layer.
     */
    this.createLayer = function() {
        var layer = new Occamy.Display.VisibleLayer(displayWidth, displayHeight);
     ...
     * 
     * @return {Occamy.Layer} The newly-created buffer.
     */
    this.createBuffer = function() {
        var buffer = new Occamy.Layer(0, 0);
        buffer.autosize = 1;
        return buffer;
    };
     ...
     * @param {Occamy.Layer} layer The source layer containing the data which
     ...
     * from the hardware cursor in that it is built into the Occamy.Display,
     * and relies on its own Occamy layer to render.
     *
     ...
     * 
     * @param {Occamy.Layer} layer The layer to resize.
     ...
     * 
     * @param {Occamy.Layer} srcLayer The Layer to copy image data from.
     * @param {Number} srcx The X coordinate of the upper-left corner of the
     *                      rectangle within the source Layer's coordinate
     *                      space to copy data from.
     * @param {Number} srcy The Y coordinate of the upper-left corner of the
     ...
     * @param {Number} srch The height of the rectangle within the source
     *                      Layer's coordinate space to copy data from.
     * @param {Occamy.Layer} dstLayer The layer to draw upon.
     * @param {Number} x The destination X coordinate.
     * @param {Number} y The destination Y coordinate.
     ...
     * 
     * @param {Occamy.Layer} layer The layer to affect.
     */
     ...
     *                      or "miter".
     * @param {Number} thickness The line thickness in pixels.
     * @param {Occamy.Layer} srcLayer The layer to use as a repeating pattern
     *                                   within the stroke.
     */
     ...
     * transform's matrix).
     * 
     * @param {Occamy.Layer} layer The layer to modify.
     ...
     * 
     * The channel mask is a Occamy-specific compositing operation identifier
     ...
     *
     * @param {Occamy.Display.VisibleLayer} layer
     ...
     *
     * @param {Occamy.Display.VisibleLayer} parent
     *     The parent to set.
     *
     * @param {Number} x
...
/**
 * Simple container for Occamy.Layer, allowing layers to be easily
 * repositioned and nested. This allows certain operations to be accelerated
 * through DOM manipulation, rather than raster operations.
 * 
 * @constructor
 * @augments Occamy.Layer
 * @param {Number} width The width of the Layer, in pixels. The canvas element
 *                       backing this Layer will be given this width.
 * @param {Number} height The height of the Layer, in pixels. The canvas element
 *                        backing this Layer will be given this height.
 */
Occamy.Display.VisibleLayer = function(width, height) {

    Occamy.Layer.apply(this, [width, height]);
     ...
     * to the Occamy protocol, and not relevant at this level.
     * 
     * @private
     * @type {Number}
     */
    this.__unique_id = Occamy.Display.VisibleLayer.__next_id++;
     ...
     * @type {Occamy.Display.VisibleLayer}
     */
    this.parent = null;
     ...
     * 
     * @param {Occamy.Display.VisibleLayer} parent The parent to set.
     * @param {Number} x The X coordinate to move to.
     * @param {Number} y The Y coordinate to move to.
 ...
 * @type {Number}
 */
Occamy.Display.VisibleLayer.__next_id = 0;
 ...
 * @constructor
 */
Occamy.InputSink = function InputSink() {

    /**
     * Reference to this instance of Occamy.InputSink.
     *
     * @private
     * @type {Occamy.InputSink}
     */
    var sink = this;
    ...
    field.addEventListener("keypress", function clearKeypress() {
        field.value = '';
    }, false);
     ...
     * added to the DOM for the Occamy.InputSink to have any effect.
     *
     * @returns {Element}
     */
    ...
    document.addEventListener("keydown", function refocusSink() {
...
/**
 * An input stream abstraction used by the Occamy client to facilitate
 * transfer of files or other binary data.
 * 
 * @constructor
 * @param {Occamy.Client} client The client owning this stream.
 * @param {Number} index The index of this stream.
 */
Occamy.InputStream = function(client, index) {
 ...
 * any installed event handlers on the given Occamy.InputStream.
 * 
 * @constructor
 * @param {Occamy.InputStream} stream
 *     The stream that JSON will be read from.
 */
Occamy.JSONReader = function guacamoleJSONReader(stream) {

    /**
     * Reference to this Occamy.JSONReader.
     *
     * @private
     * @type {Occamy.JSONReader}
     */
    var guacReader = this;

    /**
     * Wrapped Occamy.StringReader.
     *
     * @private
     * @type {Occamy.StringReader}
     */
    var stringReader = new Occamy.StringReader(stream);
    ...
    /**
     * Returns the current length of this Occamy.JSONReader, in characters.
     *
     * @return {Number}
     *     The current length of this Occamy.JSONReader.
     */
    ...
    /**
     * Returns the contents of this Occamy.JSONReader as a JavaScript
     * object.
     *
     * @return {Object}
     *     The contents of this Occamy.JSONReader, as parsed from the JSON
 ...
 *    the Occamy.Keyboard instance to have any effect.
 */
Occamy.Keyboard = function Keyboard(element) {

    /**
     * Reference to this Occamy.Keyboard.
     * @private
     */
    var guac_keyboard = this;

    /**
     * An integer value which uniquely identifies this Occamy.Keyboard
     * instance with respect to other Occamy.Keyboard instances.
     *
     * @private
     * @type {Number}
     */
    var guacKeyboardID = Occamy.Keyboard._nextID++;

    /**
     * The name of the property which is added to event objects via markEvent()
     * to note that they have already been handled by this Occamy.Keyboard.
     *
     * @private
     * @constant
     * @type {String}
     ...
     * with this Occamy.Keyboard in focus.
     * 
     * @event
     ...
     * @private
     * @constructor
     * @augments Occamy.Keyboard.KeyEvent
     ...
     */
    this.modifiers = new Occamy.Keyboard.ModifierState();
    ...
    var syncModifierStates = function syncModifierStates(e) {
        ...
        // Get state
        var state = Occamy.Keyboard.ModifierState.fromKeyboardEvent(e);
     ...
     * Occamy.Keyboard. If the Event has already been marked as handled,
     * false is returned.
     *
     * @param {Event} e
     *     The Event to mark.
     ...
     * events signalled through this Occamy.Keyboard's onkeydown and
     * onkeyup handlers.
     *
     * @param {Element|Document} element
...
/**
 * The unique numerical identifier to assign to the next Occamy.Keyboard
 * instance.
 *
 * @private
 * @type {Number}
 */
Occamy.Keyboard._nextID = 0;

/**
 * The state of all supported keyboard modifiers.
 * @constructor
 */
Occamy.Keyboard.ModifierState = function() {
 ...
 * @returns {Occamy.Keyboard.ModifierState} The current state of keyboard
 *                                             modifiers.
 */
Occamy.Keyboard.ModifierState.fromKeyboardEvent = function(e) {
    
    var state = new Occamy.Keyboard.ModifierState();
 ...
 *                        backing this Layer will be given this height.
 */
Occamy.Layer = function(width, height) {
    ...
    /**
     * Map of all Occamy channel masks to HTML5 canvas composite operation
    ...
    this.transfer = function(srcLayer, srcx, srcy, srcw, srch, x, y, transferFunction) {
        ...
        for (var i=0; i<srcw*srch*4; i+=4) {
            ...
            var src_pixel = new Occamy.Layer.Pixel(
                src.data[i],
                src.data[i+1],
                src.data[i+2],
                src.data[i+3]
            ...
            var dst_pixel = new Occamy.Layer.Pixel(
                dst.data[i],
                dst.data[i+1],
                dst.data[i+2],
                dst.data[i+3]
 ...
 */
Occamy.Layer.ROUT  = 0x2;

/**
 * Channel mask for the composite operation "atop".
 */
Occamy.Layer.ATOP  = 0x6;

/**
 * Channel mask for the composite operation "xor".
 */
Occamy.Layer.XOR   = 0xA;

/**
 * Channel mask for the composite operation "rover".
 */
Occamy.Layer.ROVER = 0xB;

/**
 * Channel mask for the composite operation "over".
 */
Occamy.Layer.OVER  = 0xE;

/**
 * Channel mask for the composite operation "plus".
 */
Occamy.Layer.PLUS  = 0xF;
 ...
 */
Occamy.Layer.Pixel = function(r, g, b, a) {
 ...
 * Occamy.Mouse instance.
 * 
 * @constructor
 * @param {Element} element The Element to use to provide mouse events.
 */
Occamy.Mouse = function(element) {

    /**
     * Reference to this Occamy.Mouse.
     * @private
     */
    var guac_mouse = this;
     ...
     * 
     * @type {Occamy.Mouse.State}
     */
    this.currentState = new Occamy.Mouse.State(
        0, 0, 
     ...
     * associated with this Occamy.Mouse.
     * 
     * @event
     * @param {Occamy.Mouse.State} state The current mouse state.
     */
     ...
     * Occamy.Mouse only, and may fail depending on browser support for
     ...
     * Occamy.Display, if the local cursor cannot be set.
     *
     * @param {HTMLCanvasElement} canvas The cursor image.
     * @param {Number} x The X-coordinate of the cursor hotspot.
     * @param {Number} y The Y-coordinate of the cursor hotspot.
 ...
 *                       button, usually part of a scroll wheel). 
 */
Occamy.Mouse.State = function(x, y, left, middle, right, up, down) {

    /**
     * Reference to this Occamy.Mouse.State.
     * @private
     */
    var guac_state = this;
 ...
 */
Occamy.Mouse.Touchpad = function(element) {

    /**
     * Reference to this Occamy.Mouse.Touchpad.
     * @private
     */
 ...
 * @param {Element} element The Element to use to provide touch events.
 */
Occamy.Mouse.Touchscreen = function(element) {

    /**
     * Reference to this Occamy.Mouse.Touchscreen.
     * @private
     */
    ...
    /**
     * Fired whenever the user moves the mouse by dragging their finger over
     * the touchscreen element. Note that unlike Occamy.Mouse.Touchpad,
...
/**
 * An object used by the Occamy client to house arbitrarily-many named
 * input and output streams.
 * 
 * @constructor
 * @param {Occamy.Client} client
 *     The client owning this object.
 *
 * @param {Number} index
 *     The index of this object.
 */
Occamy.Object = function guacamoleObject(client, index) {

    /**
     * Reference to this Occamy.Object.
     *
     * @private
     * @type {Occamy.Object}
     */
     ...
     *
     * @event
     * @param {Occamy.InputStream} inputStream
     ...
     *     is received. This callback will be provided a Occamy.InputStream
 ...
 * @type {String}
 */
Occamy.Object.ROOT_STREAM = '/';

/**
 * The mimetype of a stream containing JSON which maps available stream names
 * to their corresponding mimetype. The root stream of a Occamy.Object MUST
 * have this mimetype.
 *
 * @constant
 * @type {String}
 */
Occamy.Object.STREAM_INDEX_MIMETYPE = 'application/vnd.glyptodon.guacamole.stream-index+json';
 ...
 *
 * @constructor
 * @param {Occamy.OnScreenKeyboard.Layout} layout
 *     The layout of the on-screen keyboard to display.
 */
Occamy.OnScreenKeyboard = function(layout) {

    /**
     * Reference to this Occamy.OnScreenKeyboard.
     *
     * @private
     * @type {Occamy.OnScreenKeyboard}
     */
    var osk = this;
     ...
     *
     * @returns {Occamy.OnScreenKeyboard.Key}
    ...
    /**
     * Fired whenever the user presses a key on this Occamy.OnScreenKeyboard.
     * 
     * @event
     * @param {Number} keysym The keysym of the key being pressed.
     */
    ...
    /**
     * Fired whenever the user releases a key on this Occamy.OnScreenKeyboard.
     * 
     * @event
     * @param {Number} keysym The keysym of the key being released.
     */
     ...
     *
     * @type {Occamy.OnScreenKeyboard.Layout}
     */
    this.layout = new Occamy.OnScreenKeyboard.Layout(layout);
    ...
    /**
     * Resizes all elements within this Occamy.OnScreenKeyboard such that
     * the width is close to but does not exceed the specified width. The
     * height of the keyboard is determined based on the width.
     * 
     * @param {Number} width The width to resize this Occamy.OnScreenKeyboard
     *                       to, in pixels.
     */
     ...
     *
     * @param {Number|String|Occamy.OnScreenKeyboard.Key|Occamy.OnScreenKeyboard.Key[]} object
    ...
    var asKeyArray = function asKeyArray(name, object) {
        ...
        if (object instanceof Array) {
            ...
            for (var i=0; i < object.length; i++) {
                keys.push(new Occamy.OnScreenKeyboard.Key(object[i], name));
            }
            return keys;
        }
        ...
        if (typeof object === 'number') {
            return [new Occamy.OnScreenKeyboard.Key({
                name   : name,
                keysym : object
            })];
        }
        ...
        return [new Occamy.OnScreenKeyboard.Key(object, name)];

    };

    /**
     * Converts the rather forgiving key mapping allowed by
     * Occamy.OnScreenKeyboard.Layout into a rigorous mapping of key name
     * to key definition, where the key definition is always an array of Key
     * objects.
     *
     * @private
     * @param {Object.<String, Number|String|Occamy.OnScreenKeyboard.Key|Occamy.OnScreenKeyboard.Key[]>} keys
     ...
     *
     * @returns {Object.<String, Occamy.OnScreenKeyboard.Key[]>}
     ...
     *
     * @type {Object.<String, Occamy.OnScreenKeyboard.Key[]>}
     */
    ...
    var appendElements = function appendElements(element, object, name) {
        ...
        // If an object, append each property value
            ...
            for (i=0; i < names.length; i++) {
                appendElements(div, object[names[i]], names[i]);
            }
...
Occamy.OnScreenKeyboard.Layout = function(template) {
     ...
     *
     * @type {Object.<String, Number|String|Occamy.OnScreenKeyboard.Key|Occamy.OnScreenKeyboard.Key[]>}
     */
     ...
     * derived later via a call to resize() on the Occamy.OnScreenKeyboard.
     *
     * @type {Number}
     */
 ...
 * Occamy.OnScreenKeyboard.Key, whether that key is explicitly defined or
 * implied, and may have multiple Occamy.OnScreenKeyboard.Key if behavior
 * depends on modifier states.
 *
 * @constructor
 * @param {Occamy.OnScreenKeyboard.Key|Object} template
 *     The object whose identically-named properties will be used to initialize
 ...
 */
Occamy.OnScreenKeyboard.Key = function(template, name) {
 ...
 * @param {Number} index The index of this stream.
 */
Occamy.OutputStream = function(client, index) {
...
/**
 * Simple Occamy protocol parser that invokes an oninstruction event when
 * full instructions are available from data received via receive().
 * 
 * @constructor
 */
Occamy.Parser = function() {
     ...
     * this Occamy.Parser, executing all completed instructions at
    ...
    /**
     * Fired once for every complete Occamy instruction received, in order.
     * 
     * @event
     * @param {String} opcode The Occamy instruction opcode.
...
/**
 * A Occamy status. Each Occamy status consists of a status code, defined
 * by the protocol, and an optional human-readable message, usually only
 * included for debugging convenience.
 *
 * @constructor
 * @param {Number} code
 *     The Occamy status code, as defined by Occamy.Status.Code.
 *
 * @param {String} [message]
 *     An optional human-readable message.
 */
Occamy.Status = function(code, message) {

    /**
     * Reference to this Occamy.Status.
     * @private
     */
    var guac_status = this;

    /**
     * The Occamy status code.
     * @see Occamy.Status.Code
     * @type {Number}
     */
    this.code = code;
     ...
     * the Occamy status code into a message.
     * 
     * @type {String}
     */
...
/**
 * Enumeration of all Occamy status codes.
 */
Occamy.Status.Code = {
 ...
 * handlers on the given Occamy.InputStream.
 * 
 * @constructor
 * @param {Occamy.InputStream} stream The stream that data will be read
 *                                       from.
 */
Occamy.StringReader = function(stream) {
    ...
    /**
     * Wrapped Occamy.ArrayBufferReader.
     * @private
     * @type {Occamy.ArrayBufferReader}
     */
    var array_reader = new Occamy.ArrayBufferReader(stream);
 ...
 *                                        to.
 */
Occamy.StringWriter = function(stream) {
    ...
    /**
     * Wrapped Occamy.ArrayBufferWriter.
     * @private
     * @type {Occamy.ArrayBufferWriter}
     */
    var array_writer = new Occamy.ArrayBufferWriter(stream);
...
/**
 * Core object providing abstract communication for Occamy. This object
 * is a null implementation whose functions do nothing. Occamy applications
 * should use {@link Occamy.HTTPTunnel} instead, or implement their own tunnel based
 * on this one.
 * 
 * @constructor
 * @see Occamy.HTTPTunnel
 */
Occamy.Tunnel = function() {
     ...
     * @type {Number}
     */
    this.state = Occamy.Tunnel.State.CONNECTING;
...
/**
 * The Occamy protocol instruction opcode reserved for arbitrary internal
 ...
 * @type {String}
 */
Occamy.Tunnel.INTERNAL_DATA_OPCODE = '';
...
/**
 * Occamy Tunnel implemented over WebSocket via XMLHttpRequest.
 * 
 * @constructor
 * @augments Occamy.Tunnel
 * @param {String} tunnelURL The URL of the WebSocket tunneling service.
 */
Occamy.WebSocketTunnel = function(tunnelURL) {
    ...
    function reset_timeout() {
        ...
        receive_timeout = window.setTimeout(function () {
            close_tunnel(new Occamy.Status(Occamy.Status.Code.UPSTREAM_TIMEOUT, "Server timeout."));
     ...
     * 
     * @private
     * @param {Occamy.Status} status The status causing the connection to
     *                                  close;
     */
    function close_tunnel(status) {

        // Ignore if already closed
        if (tunnel.state === Occamy.Tunnel.State.CLOSED)
            return;

        // If connection closed abnormally, signal error.
        if (status.code !== Occamy.Status.Code.SUCCESS && tunnel.onerror)
            tunnel.onerror(status);

        // Mark as closed
        tunnel.setState(Occamy.Tunnel.State.CLOSED);
    ...
    this.sendMessage = function() {
        ...
        if (tunnel.state !== Occamy.Tunnel.State.OPEN)
            return;
         ...
         * element in a Occamy instruction.
         * 
         * @private
    ...
    this.connect = function(data) {
        ...
        tunnel.setState(Occamy.Tunnel.State.CONNECTING);
        ...
        socket.onclose = function(event) {
            close_tunnel(new Occamy.Status(parseInt(event.reason), event.reason));
        };
        
        socket.onerror = function(event) {
            close_tunnel(new Occamy.Status(Occamy.Status.Code.SERVER_ERROR, event.data));
        };
        ...
        socket.onmessage = function(event) {
            ...
            do {
                ...
                // If no period, incomplete instruction.
                    ...
                    close_tunnel(new Occamy.Status(Occamy.Status.Code.SERVER_ERROR, "Incomplete instruction."));
                ...
                if (terminator === ";") {
                    ...
                    if (tunnel.state !== Occamy.Tunnel.State.OPEN) {
                        ...
                        if (opcode === Occamy.Tunnel.INTERNAL_DATA_OPCODE)
                            tunnel.uuid = elements[0];

                        // Tunnel is now open and UUID is available
                        tunnel.setState(Occamy.Tunnel.State.OPEN);

                    }

                    // Call instruction handler.
                    if (opcode !== Occamy.Tunnel.INTERNAL_DATA_OPCODE && tunnel.oninstruction)
    ...
    this.disconnect = function() {
        close_tunnel(new Occamy.Status(Occamy.Status.Code.SUCCESS, "Manually closed."));
    };

};

Occamy.WebSocketTunnel.prototype = new Occamy.Tunnel();

/**
 * The unique ID of this version of the Occamy JavaScript API. This ID will
 ...
 * @type {String}
 */
Occamy.API_VERSION = "0.9.14";

module.exports = Occamy;
